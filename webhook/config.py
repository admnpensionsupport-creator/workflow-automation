...
SUPABASE_TABLE = os.environ.get("SUPABASE_TABLE", "Batch 1")
...
# wins. Defaults to both cohorts so opens/clicks from either campaign are
# recorded correctly without requiring a redeploy when new batches are added.
SUPABASE_TABLES = [
    t.strip()
    for t in os.environ.get("SUPABASE_TABLES", "Batch 1,Batch 2").split(",")
    if t.strip()
]
