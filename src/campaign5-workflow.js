/**
 * CAMPAIGN 5 WORKFLOW: Campbell USD & San Jose USD Educator Retirement Review
 *
 * Reads from the "Cold Email Campaign 5" table and sends the 3-email drip
 ...
 *
 * Usage:
 *   node src/campaign5-workflow.js                  # send up to 500
 *   node src/campaign5-workflow.js --limit=100      # cap at 100
 *   node src/campaign5-workflow.js --limit=5        # send only 5 (bounce test)
 *   node src/campaign5-workflow.js --dry-run        # preview only, no send
 *   node src/campaign5-workflow.js --step=1         # only send step 1
 */
...
async function main() {
  ...
  console.log('  CAMPAIGN 5 \u2014 Campbell USD & San Jose USD Retirement Review');
  ...
  if (DRY_RUN) {
    ...
    for (const c of contacts.slice(0, 5)) {
      const template = getCampaign5EmailForStep(c.sequence_step, c.name, c.id);
  ...
  if (!apiKey) throw new Error('Missing CAMPAIGN5_RESEND_API_KEY, CAMPAIGN4_RESEND_API_KEY, or RESEND_API_KEY in .env');
  ...
  console.log('');
  console.log('\u2550'.repeat(40));
  console.log('  CAMPAIGN 5 COMPLETE');
