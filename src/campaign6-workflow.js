/**
 * CAMPAIGN 6 WORKFLOW: Santa Clara USD Educator Retirement Review
 *
 * Reads from the "Cold Email Campaign 6" table and sends the 3-email drip
 * sequence to eligible contacts.
 *
 * Sequence:
 *   Step 1 → Email 1 (introduction)
 *   Step 2 → Email 2 (taxes & policy upgrades, 3+ days after step 1)
 *   Step 3 → Email 3 (call to action, 3+ days after step 2)
 *   Step 4 → done (all emails sent)
 *
 * Daily cap: 500 emails per run.
 *
 * Usage:
 *   node src/campaign6-workflow.js                  # send up to 500
 *   node src/campaign6-workflow.js --limit=100      # cap at 100
 *   node src/campaign6-workflow.js --limit=5        # send only 5 (bounce test)
 *   node src/campaign6-workflow.js --dry-run        # preview only, no send
 *   node src/campaign6-workflow.js --step=1         # only send step 1
 */

import { createClient } from '@supabase/supabase-js';
import { Resend } from 'resend';
import { getCampaign6EmailForStep, unsubscribeUrl, CAMPAIGN6_MIN_DAYS } from './utils/campaign6-templates.js';
...
async function fetchEligibleContacts(supabase, step) {
  ...
  const eligible = withEmail.filter((row) => {
    const minDays = CAMPAIGN6_MIN_DAYS[row.sequence_step] || 0;
...
async function main() {
  ...
  console.log('  CAMPAIGN 6 \u2014 Santa Clara USD Retirement Review');
  ...
  if (DRY_RUN) {
    ...
    for (const c of contacts.slice(0, 5)) {
      const template = getCampaign6EmailForStep(c.sequence_step, c.name, c.id);
  ...
  for (const contact of contacts) {
    const template = getCampaign6EmailForStep(contact.sequence_step, contact.name, contact.id);
  ...
  console.log('');
  console.log('\u2550'.repeat(40));
  console.log('  CAMPAIGN 6 COMPLETE');
