/**
 * CAMPAIGN 3 WORKFLOW: Palo Alto Educator Retirement Review
 *
 * Reads from the "Cold Email Campaign 3" table and sends the 3-email drip
 ...
 *
 * Usage:
 *   node src/campaign3-workflow.js                  # send up to 500
 *   node src/campaign3-workflow.js --limit=100      # cap at 100
 *   node src/campaign3-workflow.js --limit=5        # send only 5 (bounce test)
 *   node src/campaign3-workflow.js --dry-run        # preview only, no send
 *   node src/campaign3-workflow.js --step=1         # only send step 1
 */

import { createClient } from '@supabase/supabase-js';
import { Resend } from 'resend';
import { getCampaign3EmailForStep, unsubscribeUrl, CAMPAIGN3_MIN_DAYS } from './utils/campaign3-templates.js';
...
async function fetchEligibleContacts(supabase, step) {
  ...
  const eligible = withEmail.filter((row) => {
    const minDays = CAMPAIGN3_MIN_DAYS[row.sequence_step] || 0;
...
async function main() {
  ...
  console.log('  CAMPAIGN 3 \u2014 Palo Alto Retirement Review');
  ...
  if (DRY_RUN) {
    ...
    for (const c of contacts.slice(0, 5)) {
      const template = getCampaign3EmailForStep(c.sequence_step, c.name, c.id);
  ...
  const apiKey = process.env.CAMPAIGN3_RESEND_API_KEY || process.env.CAMPAIGN2_RESEND_API_KEY || process.env.RESEND_API_KEY;
  const fromEmail = process.env.CAMPAIGN3_EMAIL_FROM || process.env.CAMPAIGN2_EMAIL_FROM || 'tgarcia@io.pensionexpertshq.com';
  if (!apiKey) throw new Error('Missing CAMPAIGN3_RESEND_API_KEY, CAMPAIGN2_RESEND_API_KEY, or RESEND_API_KEY in .env');
  ...
  for (const contact of contacts) {
    const template = getCampaign3EmailForStep(contact.sequence_step, contact.name, contact.id);
  ...
  console.log('');
  console.log('\u2550'.repeat(40));
  console.log('  CAMPAIGN 3 COMPLETE');
