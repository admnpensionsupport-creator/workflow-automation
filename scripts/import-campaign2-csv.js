/**
 * IMPORT: Load contacts from a CSV into the "Cold Email Campaign 2" table.
 *
 * Expected CSV columns: First Name,Last Name,Email,District,County,Source
 *
 * Usage:
 *   node scripts/import-campaign2-csv.js data/campaign2-contacts.csv
 *   node scripts/import-campaign2-csv.js data/campaign2-contacts.csv --dry-run
 */
...
if (!csvPath) {
  console.error('Usage: node scripts/import-campaign2-csv.js <csv-path> [--dry-run]');
  process.exit(1);
}
...
async function main() {
  ...
  for (const r of csvRows) {
    ...
    const district = (r.District || '').trim() || null;
  ...
  if (DRY_RUN) {
    ...
    for (const row of inserts.slice(0, 5)) {
      console.log(`   ${row.name || '(no name)'} <${row.email}> \u2014 ${row.district || '(no district)'}`);
    }
    return;
  }
