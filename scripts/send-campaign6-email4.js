/**
 * CAMPAIGN 6 — Email 4: "The Final Check-In / Summer Pivot"
 *
 * Sends a custom email to all campaign6 contacts in the Cold Email Campaign 6 table.
 * Uses a campaign-specific Calendly link and email copy.
 *
 * Usage:
 *   node scripts/send-campaign6-email4.js                  # send to all campaign6 contacts
 *   node scripts/send-campaign6-email4.js --dry-run        # preview, no send
 *   node scripts/send-campaign6-email4.js --limit=5        # cap total (test run)
 *
...
async function fetchCampaign6Contacts(supabase) {
  const PAGE = 1000;
  let all = [];
  let page = 0;
  let more = true;
...
async function main() {
  console.log('════════════════════════════════════════');
  console.log('  CAMPAIGN 6 — Email 4: Final Check-In');
  ...
  let contacts = await fetchCampaign6Contacts(supabase);
  console.log(`Fetched ${contacts.length} active campaign6 contacts`);
  ...
  // Update Supabase — mark last_emailed_at
  if (sentIds.length > 0) {
