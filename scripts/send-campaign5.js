/**
 * CAMPAIGN 5 — Custom email sequence for Franklin-McKinley + Fremont + Palo Alto
 *
 * 4-email sequence:
 ...
 *   Email 4 (step 4, Mid-June) — handled by send-campaign5-email4.js
 *
 * Usage:
 *   node scripts/send-campaign5.js --step=1                 # send Email 1 to step-1 contacts
 *   node scripts/send-campaign5.js --step=2                 # send Email 2 to step-2 contacts
 *   node scripts/send-campaign5.js --step=3                 # send Email 3 to step-3 contacts
 *   node scripts/send-campaign5.js --step=1 --dry-run       # preview
 *   node scripts/send-campaign5.js --step=1 --limit=5       # test with 5
 *
...
if (!STEP || STEP < 1 || STEP > 3) {
  console.error('Usage: node scripts/send-campaign5.js --step=<1|2|3> [--dry-run] [--limit=N]');
  process.exit(1);
}
...
async function main() {
  ...
  console.log('════════════════════════════════════════');
  console.log(`  CAMPAIGN 5 — Email ${STEP}`);
  ...
  console.log(`Fetched ${contacts.length} campaign5 contacts at step ${STEP}`);
